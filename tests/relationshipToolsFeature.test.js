...
import { createIcebreakerContext } from "../src/relationshipTools/evidenceContext.js";
...
function fixture({ state = null, storage = memoryStorage(), fetchImpl, delayImpl } = {}) {
...
test("个人说明书等待五秒后展示固定模板且不调用模型接口", async () => {
  let delayMs = null;
  let calls = 0;
  const feature = fixture({
    state: completeState(),
  ...
  assert.match(textTree(feature.elements.body), /卷首语/);
  assert.match(textTree(feature.elements.body), /第一步：表层标签剥离与深度确立/);
  assert.match(textTree(feature.elements.body), /第五步：关系升华与最终宣告/);
  feature.controller.dispose();
});

test("破冰缓存和固定说明书均可直接查看且不发送多余请求", async () => {
...
test("破冰话术仍调用模型而个人说明书只展示固定模板", async () => {
  const calls = [];
  const feature = fixture({
    state: completeState(),
...
test("固定个人说明书不受模型接口故障影响", async () => {
  let calls = 0;
  const feature = fixture({
    state: completeState(),
    fetchImpl: async () => {
      calls += 1;
      throw new Error("模型接口故障");
    },
    delayImpl: async () => {},
  });
...
test("关闭个人说明书卡片会取消五秒等待", async () => {
  let signal;
  const feature = fixture({
    state: completeState(),
    delayImpl: (_duration, options) => {
  ...
  const click = feature.elements.manualButton.click();
  feature.elements.closeButton.click();
  await click;

  assert.equal(signal.aborted, true);
  assert.equal(feature.elements.dialog.open, false);
  assert.equal(feature.elements.manualButton.focused, true);
  assert.equal(feature.elements.manualButton.disabled, false);
  feature.controller.dispose();
});
